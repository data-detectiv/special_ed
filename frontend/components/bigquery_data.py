...
class BigqueryData:
    ...
    def get_table_operations(self):
        ...
        with st.expander("Actions"):
            # Save changes button (handles both new and existing rows)
            if st.button("💾 Save Changes", key=f"{self.table}_btn_save_all", type="primary"):
                try:
                    ...
                    with st.spinner("Deleting selected rows..."):
                        ...
                        for row_id in ids:
                            try:
                                ...
                                error_str = str(e)
                                if "streaming buffer" in error_str:
                                    st.error("Some rows are still being processed by BigQuery and cannot be deleted yet. Please wait a few minutes and try again.")
                                else:
                                    st.error(f"Failed to delete {self.table} {row_id}")
                        if success_count > 0:
